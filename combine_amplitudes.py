...
for looptype, tier, i_ori, ori in _list:
    amplitudes = np.load(data_dir+"%s_atoms_amplitude_%s_T%d_R%d_ori%d_0000.npy"%(\
    ...
    while True:
        try:
            amplitudes[:,1] += np.load(data_dir+"%s_atoms_amplitude_%s_T%d_R%d_ori%d_%04d.npy"%(\
    ...
    np.save(data_dir+"%s_atoms_amplitude_%s_T%d_R%d_ori%d_combined.npy"%(\
