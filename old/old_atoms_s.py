...
for i_i_file, [looptype, tier, i_file] in enumerate(filelist):
    ...
    for xyz in xyz_list:
        x, y, z = xyz[0:3]/R
        #if not (30.<x*R/a0<40. and 3.<y*R/a0<4. and 1.<z*R/a0 < 2.):
        if not x == 0.:
            continue
        print "xyz:", xyz/a0
        ...
        counter = 0 # change flag when counter gets to 4
        ...
        while True:
            ...
            if counter == 4 or (\
                s_prev != [] and np.nanmax(abs((s-s_prev)/s)) < 1.e-10):
                if justchangedflag:
                    break
                s_prev = s
                flag = (flag+1) % 2
                counter = 0
                if s_prev != [] and np.nanmax(abs((s-s_prev)/s)) < 1.e-10:
                    justchangedflag = True # only in this case do we 
                                           #set the flag
                if flag == 0:
                    n1 *= 2 
    ...
    print "done %s_atoms_s_%s_T%d_R%d_%04d.npy"%(sample, looptype, tier, R, i_file)
